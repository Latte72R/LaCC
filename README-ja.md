...
### 最適化
LaCC は AST から直接アセンブリを出す方式ではなく, MIR（中間表現）を利用した最適化を実装しています．
最適化レベルに応じて, 次の処理を行います.

- **`-O0`**
  - MIR 生成と通常のレジスタ割り当て・命令出力を行います.
  - 主要な最適化パス（インライン展開 / mem2reg / CFG 掃除）は実行しません.
  - ただし, パーサーでの定数畳み込みや，命令出力段での局所的な簡約（即値オペランドの利用など）は行います.

- **`-O1`**
  - `-O0` の処理に加えて, MIR ベースの最適化を段階的に適用します.
  - 代表的には以下を実行します.
    - 条件付きでの `static inline` 関数のインライン展開
    - copy propagation（コピー伝播）/ DCE（どこからも使われない命令の削除）
    - `cmp+setcc+jz` 系の融合
    - 未到達ブロック・未参照ラベルの削除
    - mem2reg（昇格可能なローカルをレジスタ化）
    - dead store 削除（CFG ベース）
    - vreg の詰め直し
    - 出力段階での定数畳み込み
